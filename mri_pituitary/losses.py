...
class DiceLoss2(nn.Module):
    def __init__(self, weight=(1.0, 1.0, 1.0, 1.0), size_average=True, per_image=False, dtype=None, device=None):
        super().__init__()
        factory_kwargs = {'dtype': dtype, 'device': device}
        self.size_average = size_average
        self.weight = torch.tensor(weight, **factory_kwargs)
        self.per_image = per_image
        self.eps = 1e-6

    def forward(self, input, target):

        target = target * self.weight.view(1, -1, 1, 1)

        loss = torch.zeros(1, requires_grad=True)
        for k in range(input.shape[1]):
            num = 2 * torch.sum(target[:, k] * input[:, k], dim=(1, 2))
            den = torch.sum(target[:, k], dim=(1, 2)) + torch.sum(input[:, k], dim=(1, 2))
            loss = loss + self.weight[k] * (1 - num / den).mean()

        return loss


        # dice_target = target.contiguous().view(target.shape[0], -1).float()
        # dice_output = input.contiguous().view(input.shape[0], -1)
        # intersection = torch.sum(dice_output * dice_target, dim=1)
